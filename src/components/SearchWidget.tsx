...
export function SearchWidget({ onSearchResults, onClearSearch, timelineRange, open, onOpenChange, activeQuery }: SearchWidgetProps) {
  ...
  return <div className="relative flex min-w-0 items-center">
    ...
    </div> : activeQuery ? <div className="flex h-[30px] min-w-0 items-center whitespace-nowrap rounded-[12px] bg-[#f5f3ed] text-xs font-medium text-[#3c3931]">
      <button type="button" onClick={() => onOpenChange(true)} aria-label={`Search results for ${activeQuery}`} title={`Search results for “${activeQuery}”`} className="flex h-full min-w-0 items-center gap-2 rounded-l-[12px] pl-3 pr-2 transition-colors hover:bg-[#efece4]"><Search size={15} className="shrink-0" /><span>{activeQuery}</span></button>
      <button type="button" onClick={handleClear} aria-label={`Clear search for ${activeQuery}`} title="Clear search" className="flex h-full w-8 shrink-0 items-center justify-center rounded-r-[12px] text-[#77736b] transition-colors hover:bg-[#e9e5dc] hover:text-[#0f1524]"><X size={14} /></button>
    </div> : <button type="button" onClick={() => onOpenChange(true)} aria-label="Open artwork search" title="Search or click anything that catches your eye" className="flex h-[30px] w-[min(58vw,360px)] items-center justify-start gap-2 whitespace-nowrap rounded-[12px] bg-[#f5f3ed] px-3 text-xs font-medium text-[#77736b] transition-colors hover:bg-[#efece4] hover:text-[#3c3931] sm:w-[360px]"><Search size={15} className="shrink-0" /><span className="sm:hidden">Search for anything</span><span className="hidden sm:inline">Search or click anything that catches your eye</span></button>}
  </div>
}
