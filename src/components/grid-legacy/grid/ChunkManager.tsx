...
interface ChunkManagerProps {
  ...
  /** Optional finite artwork set to render instead of fetching random chunks */
  artworks?: Artwork[]
  /** Whether more artworks can be appended to the finite artwork set */
  hasMoreArtworks?: boolean
  /** Whether an additional artwork page is already loading */
 ...
 */
function generateChunkImagesFromArtworks(
  chunkX: number,
  chunkY: number,
  artworks: Artwork[],
  fillChunk: boolean
  ...
  let chunkArtworks = validArtworks
  ...
  if (fillChunk) {
    ...
    for (let i = 0; i < CHUNK_SIZE; i++) {
      ...
      chunkArtworks.push(validArtworks[i % validArtworks.length]!)
    }
  }
  
  return chunkArtworks.map((artwork, i) => {
...
function createChunk(
  ...
  if (artworks && artworks.length > 0) {
    images = generateChunkImagesFromArtworks(chunkX, chunkY, artworks, fillChunk)
  }
...
function getSpiralChunkCoordinates(count: number): ChunkCoordinates[] {
  const coordinates: ChunkCoordinates[] = [{ x: 0, y: 0 }]
  let x = 0
  let y = 0
  let deltaX = 1
  let deltaY = 0
...
const ChunkManager = memo(function ChunkManager({
  ...
  const staticChunkDataMap = useMemo(() => {
    ...
    const validArtworks = artworks.filter((artwork) => Boolean(
      artwork.primaryImageSmall
      ?? artwork.imageUrl
      ?? artwork.primaryImage
      ?? artwork.originalImageUrl
    ))
    const coordinates = getSpiralChunkCoordinates(
      Math.ceil(validArtworks.length / CHUNK_SIZE)
    )

    for (let offset = 0; offset < validArtworks.length; offset += CHUNK_SIZE) {
      const chunkIndex = offset / CHUNK_SIZE
      const coordinate = coordinates[chunkIndex]!

      data.set(`${coordinate.x},${coordinate.y}`, {
        artworks: validArtworks.slice(offset, offset + CHUNK_SIZE),
        loading: false,
        error: null,
      })
    }
  ...
  const isStaticGrid = artworks !== undefined
  const chunkDataMap = isStaticGrid ? staticChunkDataMap : fetchedChunkDataMap
  ...
  const loadableChunks = useMemo(
    () => isStaticGrid
      ? chunksToLoad.filter((coord) => staticChunkDataMap.has(`${coord.x},${coord.y}`))
      : chunksToLoad,
    [chunksToLoad, isStaticGrid, staticChunkDataMap]
  )
  ...
  const createChunksFromCoordinates = useCallback((coords: ChunkCoordinates[]) => {
    ...
    for (const coord of coords) {
      ...
      if (!chunks.has(chunkKey)) {
        try {
          const newChunk = createChunk(coord.x, coord.y, chunkDataMap, !isStaticGrid)
  ...
  }, [chunks, chunkDataMap, isStaticGrid])
   ...
   */
  useEffect(() => {
    if (loadableChunks.length > 0) {
      void loadChunksRef.current(loadableChunks)
    }
  }, [loadableChunks])
  ...
  useEffect(() => {
    ...
    const needsMoreArtworks = chunksToLoad.some(
      (coord) => !staticChunkDataMap.has(`${coord.x},${coord.y}`)
    )

    if (needsMoreArtworks) {
      void onLoadMoreArtworks()
    }
  }, [
    chunksToLoad,
    hasMoreArtworks,
  ...
  return (
    <GridRenderer
      ...
      chunksToLoad={renderedChunksToLoad}
