...
const ChunkManager = memo(function ChunkManager({
  ...
  useEffect(() => {
    if (isInitialized && !isDragging) {
      if (DEBUG_LOGGING) {
        console.log('🚀 ChunkManager: Initial virtualization trigger')
      }
  ...
  if (DEBUG_LOGGING) {
    console.log(`🎛️ ChunkManager: Rendering ${chunks.size} chunks, ${visibleChunks.length} visible, ${chunksToLoad.length} to load`)
  }
