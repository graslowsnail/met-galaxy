...
export function DraggableImageGrid({
  ...
  // Calculate viewport center for hybrid approach
  const viewportCenter = useMemo(() => ({
    x: -translate.x + size.width / 2,
    y: -translate.y + size.height / 2
  }), [translate, size])
  
  const { upsertChunk, snapshotPlaced, getStats, pruneTo, getPlaced } = useColumnCarryover({ viewportCenter })
  ...
  useEffect(() => {
    ...
    // HYBRID: Aggressively prune chunks based on distance from viewport
    pruneTo(visibleKeys)
    
    // Reset loaded chunks tracking to only keep nearby chunks
    // This prevents memory accumulation from long scrolling sessions
    const nearbyChunks = new Set<string>()
    for (const coord of visible) {
      nearbyChunks.add(`${coord.x}:${coord.y}`)
    }
    setLoadedChunks(nearbyChunks)
  }, [visible, pruneTo])
